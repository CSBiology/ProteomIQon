...
module PSMStats = 
    ...
    type AppliedModel = {
        ...
        Model           : PercolatorIn -> PSMPrediction
        }
    ...
    let pepValueCalcAndProteinInference (processParams:PSMStatisticsParams) (outputDir:string) (cn:SQLiteConnection) (psms:string) =
        ...
        let trainModel positives' negatives' =
            ...
            let predF = ctx.Model.CreatePredictionEngine<PercolatorIn,PSMPrediction>(model)
            let predict psm = 
                psm 
                |> predF.Predict
            predict 
        ...
        let applyModel iteration (trainedModel: PercolatorIn -> PSMPrediction) psms =
            let bestPSMPerScan = 
                psms
                |> Array.groupBy (fun x -> x.ScanNr)
                |> Array.map (fun (psmId,psms) -> 
        ...
        let initModel = applyModel 0 predict percolatorIn
        let refinedModel = 
            [0 .. 3]
            |> List.fold (fun (acc:AppliedModel) i -> 
                    ...
                    let predict = trainModel (positives'|> Map.toArray |> Array.map snd |> Set.ofArray)  negatives' 
                    let tmp = applyModel i predict percolatorIn                    
