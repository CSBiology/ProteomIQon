...
module console1 =
    ...
    let main argv = 
        ...
        let p = 
            Json.ReadAndDeserialize<Dto.LabelFreeQuantificationParams> p
            |> Dto.LabelFreeQuantificationParams.toDomain
        let files = 
            parsePaths (fun path -> Directory.GetFiles(path,("*.quantAndProt"))) i
            |> Array.ofSeq
        LabelFreeProteinQuantification.labelFreeQuantification p o files
        logger.Info "Done"
        0
