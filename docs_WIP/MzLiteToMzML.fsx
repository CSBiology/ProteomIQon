...
# mzlite to mzMl Conversion
**Disclaimer** this tool converts [mzlite](https://github.com/CSBiology/MzIO/blob/developer/src/MzIO.SQL/MzIOSQL.fs) to [mzML](https://www.psidev.info/mzML)
if you want to convert the other way around please visit the [MzMLToMzlite]({{root}}tools/MzMLToMzLite.html) documentation. 
...
The tool MzLiteToMzML allows to convert mzlite files to mzML files. Additionally, it allows the user to perform peak picking or filtering of mass spectra.
...
Parameters are handed to the cli tool as a .json file. you can download the default file [here](https://github.com/CSBiology/ProteomIQon/blob/master/src/ProteomIQon/defaultParams/MzLiteToMzMLParams.json), 
...
let defaultMzLiteToMzMLParams :Dto.PreprocessingParams =   
    {
...
(**
	proteomiqon-mzlitetomzml -i "path/to/your/run.mzlite" -o "path/to/your/outDirectory" -p "path/to/your/params.json"
*)

(**
It is also possible to call the tool on a list of .mzlite files. If you have a mulitcore cpu it is possible to convert multiple runs in parallel using the -c flag:
*)

(**
	proteomiqon-mzlitetomzml -i "path/to/your/run1.mzlite" "path/to/your/run2.mzlite" "path/to/your/run3.mzlite" -o "path/to/your/outDirectory" -p "path/to/your/params.json" -c 3
*)
