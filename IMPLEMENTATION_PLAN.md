...
| P1-2 Bounded peak cache | Implemented (code), partial validation complete | `src/PSMBasedQuantificationTIMs/PSMBasedQuantificationTIMs.fs:597`, `src/PSMBasedQuantificationTIMs/PSMBasedQuantificationTIMs.fs:611`, `src/PSMBasedQuantificationTIMs/PSMBasedQuantificationTIMs.fs:1374` | Build passes; tuned under memory-safe probes, full-run still blocked by memory growth (~19.6 GB observed) | Keep default bounded LRU with max=700; pair with memory-containment work before full parity run |
