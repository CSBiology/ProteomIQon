...
every source peptide ion in the target run. [AlignmentBasedQuantification]({{root}}tools/AlignmentBasedQuantification.html) then extracts and quantifies the
...
|------|---------|------------|
| `-i` | one or more `.quant` files of the target runs, or a directory that is searched for `*.quant` | [PSMBasedQuantification]({{root}}tools/PSMBasedQuantification.html) or [PSMBasedQuantificationTIMs]({{root}}tools/PSMBasedQuantificationTIMs.html) |
| `-ii` | one or more `.quant` files of the source runs, or a directory that is searched for `*.quant` | [PSMBasedQuantification]({{root}}tools/PSMBasedQuantification.html) or [PSMBasedQuantificationTIMs]({{root}}tools/PSMBasedQuantificationTIMs.html) |
...
itself has already quantified. [AlignmentBasedQuantStatistics]({{root}}tools/AlignmentBasedQuantStatistics.html) uses that overlap as training
